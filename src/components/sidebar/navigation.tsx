...
const Navigation = () => {
  ...
  return (
    <aside className="w-full h-full bg-white text-sm">
      {/* Home link */}
      <Link href="/">
        <div
          className={cn(
            'flex items-center gap-2 p-2 rounded-md hover:bg-neutral-100 transition font-medium text-slate-700',
            pathname === '/' && 'bg-neutral-100 text-primary'
          )}
        >
          {pathname === '/' ? <GoHome className="size-5" /> : <GoHome className="size-5" />}
          <span>Home</span>
        </div>
      </Link>
      ...
      {sections.map((section) => (
          ...
          <p className="px-2 text-muted-foreground font-bold">{section.title}</p>
